...
class HMAPIHandler {
    ...
    parseQuery(query) {
        ...
        terms.forEach((term, i) => {
            ...
            if (term.split("").filter((n) => n === ":").length > 1) {
                throw new error_1.HMInvalidQueryError("不合法的标签，请使用namespace:tag的格式");
            }
            if (term.includes(":")) {
                const splits = term.split(":");
                const left = splits[0].replace(/^-/, "");
                if (constant_1.namespaces.includes(left)) {
                    namespace = left;
                }
                else {
                    throw new error_1.HMInvalidQueryError("不合法的namespace");
                }
                if (!splits[1])
                    throw new error_1.HMInvalidQueryError("不合法，标签为空");
            ...
            if (or_previous || or_next) {
                if (term.match(/^-/))
                    throw new error_1.HMInvalidQueryError("不合法，或搜索中只能使用正向关键词");
    ...
    isSingleTag(term) {
        ...
        if (parsed.negative_terms.length === 0 &&
    ...
    async getSingleTagSearchPage({ state, page, }) {
        return await (0, hitomi_1.get_galleryids_and_count)({
            state,
    ...
    async search(options) {
        ...
        if (!options.term.trim() && options.orderbydirection === "desc") {
        ...
        else if (parsed.negative_terms.length === 0 &&
            parsed.or_terms.length === 0 &&
            parsed.positive_terms.length === 1 &&
            parsed.positive_terms[0].namespace &&
            ...
            await (0, hitomi_1.update_galleries_index_version)();
            const gids = await this.multiTagSearch(options);
            const rgids = options.orderbydirection === "random"
                ? shuffleArray(gids)
