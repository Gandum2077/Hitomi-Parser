...
exports.get_gallery_detail = exports.get_thumbnail_url_from_hash = exports.get_image_srcs = exports.update_galleries_index_version = exports.get_galleryblocks = exports.get_galleryids_and_count = exports.get_galleryids_from_state = exports.get_galleryids_for_query_without_namespace = void 0;
...
async function get_url_at_range(url, range) {
    ...
    const resp = await (0, request_1.fakeFetch)(url, { headers });
    if (resp.status !== 200 && resp.status !== 206) {
        throw new error_1.HMAPIError(resp.status);
    }
...
exports.get_galleryids_for_query_without_namespace = get_galleryids_for_query_without_namespace;
/**
 * 根据当前状态生成Nozomi地址
 * @param state 当前状态
...
exports.get_galleryids_from_state = get_galleryids_from_state;
async function get_galleryids_and_count({ range, state, }) {
    const headers = { referer: refererUrl, range };
...
exports.get_galleryids_and_count = get_galleryids_and_count;
async function get_single_galleryblock(gid) {
    const url = "https://" + domain + "/" + `galleryblock/${gid}.html`;
    const resp = await (0, request_1.fakeFetch)(url, { headers: { referer: refererUrl } });
    return (0, parser_1.parseGalleryBlockInfo)(await resp.text());
}
async function get_galleryblocks(gids) {
    if (gids.length > 25)
        throw new Error("Be careful: too many blocks");
    return await Promise.all(gids.map((n) => get_single_galleryblock(n)));
}
exports.get_galleryblocks = get_galleryblocks;
/**
 * 获取索引版本号
...
exports.update_galleries_index_version = update_galleries_index_version;
...
exports.get_image_srcs = get_image_srcs;
/**
 * 构建缩略图（small / big）URL
...
exports.get_thumbnail_url_from_hash = get_thumbnail_url_from_hash;
async function get_gallery_detail(gid) {
    ...
    return (0, parser_1.parseGalleryDetail)(await resp.text());
}
exports.get_gallery_detail = get_gallery_detail;
